...
The reusable PR-evaluation workflow SHALL set an explicit timeout for its evaluate job from the optional org-level Actions variable `PANOPTICON_LLM_JOB_TIMEOUT_MINUTES`, using 20 minutes when the variable is unset. The setup guide SHALL document that the variable accepts a whole number from 10 through 60 and is evaluated by GitHub Actions before the job starts.
