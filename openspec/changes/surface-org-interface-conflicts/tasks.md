...
- [ ] 2.1 Render every participating repository's interface resources in the
  organization document, including single-repository resources, while retaining
  external-only dependency rendering.
- [ ] 2.2 Add the conditional `## Detected interface conflicts` summary and
  derive per-resource conflict targets from the compiled index.
- [ ] 2.3 Render interface resources as Mermaid nodes; style conflicted nodes
...
- [ ] 3.2 Add organization-diagram tests for standalone interfaces, the exact
