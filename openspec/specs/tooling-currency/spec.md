...
Neither the workflow-ref alignment check nor the skills and tooling drift check
SHALL have an entry
in the org config's check-type/gating mechanism, and neither SHALL ever fail the
PR workflow,
...
child repo with no
instance repo clone (`python3 -m panopticon.sync` or equivalent), that fetches
the instance repo's
current default-branch copies of the child's downloaded skills and vendored
local-tooling modules
and overwrites the child repo's copies unconditionally. The script SHALL NOT
protect any file at
the child-repo layer — the user's own review of the resulting `git diff`/`git
status` before
committing is the only safety mechanism, matching the trust model the bootstrap
installer's existing
...
report which files
would change (using the same content-based comparison the GitHub API already
exposes — a git blob
hash comparison, not timestamps) and SHALL NOT write any file.

#### Scenario: Default run overwrites unconditionally

- **WHEN** the sync script is run with no flags in a child repo whose skills or
  tooling have drifted
  from the instance repo
- **THEN** every drifted file is overwritten in place with the instance repo's
