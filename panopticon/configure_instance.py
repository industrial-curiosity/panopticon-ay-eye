...
def configure(instance_root, provider, names):
    ...
    variables = {
        key: names.get(key, default)
        for key, default in definition["variables"].items()
    }
    llm = provider_config(provider, secrets, variables)
...
def main(argv=None):
    ...
    try:
        llm = configure(args.instance_root, args.provider, names)
