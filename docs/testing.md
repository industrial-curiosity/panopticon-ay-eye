...
| `tests/test_organization_template.py` | Offline profile validation and generation for direct-OIDC and instance-managed synthetic profiles; trusted provider loading, explicit child caller rendering, broker branch/tag/SHA validation, protection ownership including rejection of derived paths declared as organization debt, deterministic output, public fixture safety, read-only apply preview, stale preimage rejection, undeclared-collision rejection, and preservation of unrelated instance files |
...
| `tests/test_drift.py` | Doc-drift path classification and planner validation, metadata-only clean verdicts, isolated batch evaluator prompts, aggregate clean/stale results, opt-in secret-safe scope/planning/batch/evaluation debug traces, safe stage-aware timeout recovery, verdict parsing, report formatting, action collection, and the CLI exit-code contract. |
...
| `tests/test_tooling_currency.py` | Advisory-only tooling-currency PR check (`panopticon/tooling_currency.py`, CI only): `check_workflow_ref`'s ref-resolution against a stubbed `git ls-remote`/`rev-parse` runner (aligned, behind, deleted, and an unparseable caller-workflow file); skills/tooling content drift; instance-excluded versus child-only unmanaged Python-module warnings; state-file exclusion; one aggregated `::warning::` plus file-level step-summary details; and `main()`'s always-`0` exit code (real `git` subprocess calls against a real local git repo used as its own remote, not stubbed, to exercise the actual ref-resolution path end-to-end). |
...
| `tests/test_recovery.py` | Runtime recovery output for unconfigured instances, missing provider inputs, stale provider callers, and gate-3 credential failures/timeouts, including source-safe provider configuration references, child-bootstrap commands, configured missing names, per-child identity scope, fixed action paths, and private-instance/custom-branch context. |
...
- `tests/test_provider_workflows.py` verifies the Bedrock dispatch/action wiring, one workflow-level fixed-path binding for inline recovery, fallback markers, the getting-started link to the authoritative setup guide, and public placeholder safety for the credential-action example.
